...
<table class="table2">
	<tbody>
					...
					<?php
						...
						$search = $_GET['search'];
						$query = $conn->query("SELECT foodpacks.name3 FROM `foodpacks` WHERE EXISTS (SELECT * FROM users WHERE userID = foodpacks.userID AND fullname LIKE '%$search%' AND purok = ('$purok2') AND barangay = ('$barangay'));") or die(mysqli_error());
