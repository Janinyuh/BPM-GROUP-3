...
<!-- start table ---->
  ...
  <tbody>
					<?php
						...
						$query = $conn->query("SELECT name2, amount, date_claimed FROM `ayuda` WHERE userID = ('$userID2');") or die(mysqli_error());
...
<table class="table2">
	 <tbody>
					...
					<?php
						require 'Connection/db_conn.php';
						$query = $conn->query("SELECT ayuda.name2 FROM `ayuda` WHERE EXISTS (SELECT purok FROM users WHERE userID = ayuda.userID AND purok = ('$purok2'));") or die(mysqli_error());
						while($fetch = $query->fetch_array()){
					?>
